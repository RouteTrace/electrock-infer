...
__global__ void silu_and_mul_kernel(
  ...
  for (int64_t idx = threadIdx.x; idx < d; idx += blockDim.x) {
    ...
    const scalar_t gate_val = LDG(&input[bid * 2 * d + idx]);
    const scalar_t up_val = LDG(&input[bid * 2 * d + d + idx]);
...
__global__ void silu_and_mul_d7168_float4_kernel(
  ...
  for (int64_t i = threadIdx.x; i < d_vec; i += blockDim.x) {
    ...
    const float4 gate_vec = LDG(&input_vec[input_offset + i]);
    const float4 up_vec = LDG(&input_vec[input_offset + d_vec + i]); // gate 和 up 相隔 d (即 d_vec 个 float4)
